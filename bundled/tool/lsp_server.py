...
LSP_SERVER = server.LanguageServer(
    name="JAX Language Server", version="<server version>", max_workers=MAX_WORKERS
)
...
def _get_severity(*_codes: str) -> lsp.DiagnosticSeverity:
...
def _run_tool_on_document(
    ...
    if use_path:
        ...
        with utils.substitute_attr(sys, "path", sys.path[:]):
            try:
                # TODO: `utils.run_module` is equivalent to running `python -m jaxls`.
...
def _run_tool(extra_args: Sequence[str]) -> utils.RunResult | jsonrpc.RpcRunResult:
    """Runs tool."""
    # deep copy here to prevent accidentally updating global settings.
